...
const RPC_URL = Deno.env.get('ARBITRUM_SEPOLIA_RPC') || 'https://sepolia-rollup.arbitrum.io/rpc';
const PINATA_JWT = Deno.env.get('PINATA_JWT') || '';
const PINATA_GATEWAY = Deno.env.get('PINATA_GATEWAY') || 'https://gateway.pinata.cloud/ipfs';
const EXPLORER_BASE = Deno.env.get('BLOCK_EXPLORER_BASE') || 'https://sepolia-explorer.arbitrum.io/tx/';
...
const getExplorerUrl = (txHash: string) => `${EXPLORER_BASE}${txHash}`;

async function uploadCertificateToIPFS(certificate: any) {
  ...
  try {
    const res = await fetch('https://api.pinata.cloud/pinning/pinJSONToIPFS', {
      ...
      body: JSON.stringify({
        pinataMetadata: { name: `donation-${certificate.txHash}` },
        pinataContent: certificate,
      }),
    });

    if (!res.ok) {
      console.error('Failed to upload certificate to IPFS', await res.text());
      return null;
    }
