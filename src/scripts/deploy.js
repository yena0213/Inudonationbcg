...
async function main() {
  ...
  // 트랜잭션 정보
  console.log("📊 Deployment transaction hash:", donationVillage.deployTransaction.hash);
  ...
  for (let i = 1; i <= campaignCount; i++) {
    ...
    console.log("   Goal:", ethers.utils.formatEther(campaign.goalAmount), "ETH");
  ...
  const deploymentInfo = {
    ...
    contractAddress: donationVillage.address,
    deployer: deployer.address,
    deploymentTime: new Date().toISOString(),
    transactionHash: donationVillage.deployTransaction.hash,
  ...
  if (hre.network.name === "arbitrumSepolia") {
    ...
    console.log(`npx hardhat verify --network arbitrumSepolia ${donationVillage.address}`);
  }
  
  // 환경 변수 가이드
  console.log("\n📝 Add these to your .env file:");
  console.log(`VITE_CONTRACT_ADDRESS=${donationVillage.address}`);
