...
interface DonationModalProps {
  campaign: Campaign;
  onClose: () => void;
  onConfirm: (amount: number, txHash: string, certificateUrl?: string) => void;
  userWallet: string;
}
...
export function DonationModal({ campaign, onClose, onConfirm, userWallet }: DonationModalProps) {
  ...
  const handleDonate = async () => {
    ...
    try {
      ...
      if (!isContractDeployed()) {
        ...
        const verification = await runVerification(mockTxHash);
        const certificateLink = verification?.certificateUrl || certificateUrl || undefined;
        ...
        setStatus('success');
        setTimeout(() => {
          onConfirm(donationAmount, mockTxHash, certificateLink);
        }, 1500);
      ...
      if (receipt.status === 1) {
        const verification = await runVerification(tx.hash);
        const certificateLink = verification?.certificateUrl || certificateUrl || undefined;
        setStatus('success');
        setTimeout(() => {
          onConfirm(donationAmount, tx.hash, certificateLink);
        }, 2000);
      } else {
  ...
  const runVerification = async (hash: string) => {
    try {
      const result = await verifyDonation(hash, userWallet);
              ...
              <div className="space-y-4">
                ...
                {certificateUrl && (
                  <div className="p-4 bg-indigo-50 rounded-2xl border-2 border-indigo-200">
                    <p className="text-indigo-700 text-sm mb-1">기부 증명서 (IPFS)</p>
                    <a
                      href={certificateUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-indigo-700 text-xs underline break-all"
                    >
                      {certificateUrl}
                    </a>
                    <p className="text-indigo-600 text-xs mt-2">
                      🔗 IPFS에 업로드된 증명서를 통해 DID 기반으로 검증할 수 있어요
                    </p>
                  </div>
                )}
              </div>
